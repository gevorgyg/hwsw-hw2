...
class Colony:
    ...
    def command_table(self):
        return {
            ...
            "predict_population": self.predict_population,
    ...
    def predict_population(self, args):
        days = self._int(args, "days")
        entries = sorted(self.census.items())
        growth = 0
        if len(entries) >= 2 and entries[-1][0] > entries[0][0]:
            ...
            growth = (((last["ants"] + last["larvae"])
                      ...
                      // (last_day - first_day))
        current = len(self.ants) + len(self.larvae)
        caps = (("food", self.food),
                ("nurses", self._nursing_capacity() - len(self.larvae)),
                ("chambers", self._available_chambers()))
        limiting = min(caps, key=lambda item: item[1])[0]
        forecast = [{"day": self.day + i, "population": current + growth * i}
    ...
    def _emit(self, event_type):
        # in-process event bus: the same subscriptions the FaaS gateway
        # holds in its table, delivered as direct calls instead of a queue
        for subscriber in {"population_changed": [self._record_census]}[event_type]:
            subscriber()

    def _record_census(self):
        self.census[self.day] = {"ants": len(self.ants),
                                 "larvae": len(self.larvae)}
