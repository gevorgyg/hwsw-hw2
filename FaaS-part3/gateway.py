...
from functions.state_store import (EVENTS_PATH, STATE_DIR, initial_slices,
                                   save)
from shared.interpreter import CommandError

FUNCTIONS_DIR = Path(__file__).resolve().parent / "functions"

# _record_census refreshes the day's population snapshot after anything that
# can change the head count; it runs last so it sees the settled state.
SUBSCRIPTIONS = {
    "day_advanced": ["_open_chambers", "_record_census"],
    "colony_overrun": ["_wipe_stores", "_wipe_brood", "_record_census"],
    "population_changed": ["_record_census"],
}
